...
def check_git_status():
    ...
    try:
        ...
        ), 'skipping check (Docker image)'  # not Path('/.dockerenv').exists()
        assert check_online(), 'skipping check (offline)'

        cmd = 'git fetch && git config --get remote.origin.url'
        url = subprocess.check_output(
            cmd, shell=True).decode().strip().rstrip('.git')  # github repo url
        ...
        print(s.encode().decode('ascii', 'ignore') if platform.system() ==
              'Windows' else s)
...
def check_file(file):
    ...
    if os.path.isfile(file) or file == '':
        ...
        assert len(
            files
        ) == 1, "Multiple files match '%s', specify exact path: %s" % (
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket=''):
    ...
    with open('evolve.txt', 'a') as f:  # append result
        f.write(c + b + '\n')
    x = np.unique(
        np.loadtxt('evolve.txt', ndmin=2), axis=0)  # load unique rows
    ...
    if bucket:
        os.system(
            'gsutil cp evolve.txt %s gs://%s' % (yaml_file, bucket))  # upload
